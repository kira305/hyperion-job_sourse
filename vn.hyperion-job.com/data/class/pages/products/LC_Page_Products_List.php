...
class LC_Page_Products_List extends LC_Page_Ex {
    ...
    public function init() {
        ...
        $_SESSION['lastListUrl'] = $_SERVER['REQUEST_URI'];
    }
    ...
    public function action() {
        ...
        $this->prevPage = $_REQUEST['prevPage'];
        ...
        $this->arrSearchData = array(
            ...
            'position' => $this->arrForm['position'],
            'region' => $this->arrForm['region'],
            'city' => $this->arrForm['city'],
            'salary_type' => $this->arrForm['salary_type'],
            'currency' => $this->arrForm['currency'],
            'salary_range' => $this->arrForm['salary_range'],
            'welfare' => $this->arrForm['welfare']
        );
        ...
        switch ($this->getMode()) {
            case 'cart':
//                CheckedItems::addCookie($_POST['product_id']);
                if ($objCustomer->isLoginSuccess(true)) {
                    $objProduct->doAddFavorite($_GET['product_id'], $objCustomer->getValue('customer_id'), $objQuery);
                } else {
        ...
        $this->arrProducts = $this->lfGetProductsList($arrSearchCondition, $this->disp_number, $this->objNavi->start_row, $objProduct, $objCustomer);
    ...
    public function lfInitParam(&$objFormParam) {
        ...
        $objFormParam->addParam('職種', 'category_id', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('ポジション', 'position', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('都道府県', 'region', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('市区町村', 'city', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('給与区分', 'salary_type', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('通貨', 'currency', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('金額', 'salary_range', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('こだわり条件', 'welfare', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('prevPage', 'prevPage');
        // カートイン
     ...
     */
    public function lfGetProductsList($searchCondition, $disp_number, $startno, &$objProduct, &$objCustomer) {
        $objQuery = & SC_Query_Ex::getSingletonInstance();
        ...
        switch ($this->orderby) {
            case 'hot':
                $col = 'MAX(T2.product_status_id)';
                $from = "dtb_product_status T2";
            ...
            case 'salary':
                ...
                $rate = $arrInfo['exchange_rate'];
                $objQuery->setOrder("IF(currency=1, salary_max, salary_max*".$rate.") DESC, IF(currency=1, salary_min, salary_min*".$rate.") DESC");
                break;
            // 販売価格が安い順
            case 'price':
        ...
        if (count($arrProducts) > 0) {
            foreach ($arrProducts as $key => $val) {
                $jobImg = $arrProducts[$key]['main_large_image'];
                $base64 = '';
                if (!empty($jobImg)) {
                ...
                $arrProducts[$key]['base64_image'] = ($base64 != '') ? $base64 : IMAGE_SAVE_URLPATH . 'noimage_main.png';
                $is_favorite = SC_Helper_DB_Ex::sfDataExists('dtb_customer_favorite_products', 'customer_id = ? AND product_id = ?', array($objCustomer->getValue('customer_id'), $val['product_id']));
                $arrProducts[$key]['is_favorite'] = !empty($is_favorite) ? true : false;
            }
        }
        return $arrProducts;
    }
    ...
    public function lfGetSearchCondition($arrSearchData) {
        ...
        foreach ($names as $val) {
            if (strlen($val) > 0) {
                $searchCondition['where'] .= ' AND ( alldtl.name ILIKE ? OR alldtl.comment3 ILIKE ? OR alldtl.work_location ILIKE ? OR alldtl.main_list_comment ILIKE ? OR alldtl.main_comment ILIKE ? OR alldtl.name_vn ILIKE ? OR alldtl.work_location_vn ILIKE ? OR alldtl.main_list_comment_vn ILIKE ? OR alldtl.main_comment_vn ILIKE ? ) ';
        ...
        $arrKey = array(0 => 'region', 1 => 'city', 2 => 'welfare', 3 => 'position', 4 => 'category_id');
        if ($arrSearchData['employment_status']) {
            if (is_array($arrSearchData['employment_status'])) {
                $arrKey[] = 'employment_status';
            } else {
                $searchCondition['where'] .= ' AND alldtl.employment_status = ? ';
                $searchCondition['arrval'][] = $arrSearchData['employment_status'];
            }
        }

        foreach ($arrKey as $key) {
            if ($arrSearchData[$key] != '' && count($arrSearchData[$key]) > 0) {
                if (count($arrSearchData[$key]) == 1) {
                    foreach ($arrSearchData[$key] as $status) {
                        $searchCondition['where'] .= ' AND alldtl.' . $key . ' ILIKE ? ';
                        $searchCondition['arrval'][] = "%$status%";
                    }
                } else {
                    $max = count($arrSearchData[$key]);
                    $i = 0;
                    foreach ($arrSearchData[$key] as $status) {
                        $i++;
                        if ($i == 1)
                            ...
                            $searchCondition['where'] .= ' OR alldtl.' . $key . ' ILIKE ? )';
                        else
                            $searchCondition['where'] .= ' OR alldtl.' . $key . ' ILIKE ? ';
                        $searchCondition['arrval'][] = "%$status%";
                    }
        ...
        if ($arrSearchData['salary_type']) {
            $searchCondition['where'] .= ' AND alldtl.salary_type = ? ';
            $searchCondition['arrval'][] = $arrSearchData['salary_type'];
        }

        $objDb = new SC_Helper_DB_Ex();
        $arrInfo = $objDb->sfGetBasisData();
        $rate = $arrInfo['exchange_rate'];
        if ($arrSearchData['currency'] == 1)
            $rate = 1 / $arrInfo['exchange_rate'];

        if ($arrSearchData['salary_range']) {
            $masterData = new SC_DB_MasterData_Ex();
            $arrSalaryRange = $masterData->getMasterData('mtb_salary_range');
            $searchCondition['where'] .= ' AND (';
            if (is_array($arrSearchData['salary_range'])) {
                foreach ($arrSearchData['salary_range'] as $k => $range) {
                    if($k > 0)
                        $searchCondition['where'] .= ' OR ';
                    $searchCondition['where'] .= '(';
                    $this->buildSalaryQuery($range, $searchCondition, $arrSearchData, $rate, $arrSalaryRange);
                    $searchCondition['where'] .= ')';
                    $k++;
                }
            } else {
                $this->buildSalaryQuery($arrSearchData['salary_range'], $searchCondition, $arrSearchData, $rate, $arrSalaryRange);
            }
            $searchCondition['where'] .= ')';
        }
        ...
        if ($this->target > 0) {
            $searchCondition['where'] .= ' AND target = ?';
            $searchCondition['arrval'][] = $this->target;
        }
    ...
    public function buildSalaryQuery($id, &$searchCondition, $arrSearchData, $rate, $arrSalaryRange) {
        if ($id > 10000) {
            $arrSearchData['salary_min'] = $arrSalaryRange[substr($id, 0, 3)];
        } else if ($id > 1000) {
            $arrSearchData['salary_max'] = $arrSalaryRange[substr($id, 0, 3)];
        } else {
            $arrSearchData['salary_min'] = $arrSalaryRange[$id];
            $arrSearchData['salary_max'] = $arrSalaryRange[$id + 1];
        }
        if ($arrSearchData['salary_min'] && $arrSearchData['salary_max']) {
            $searchCondition['where'] .= '
                alldtl.currency = ? AND (
                    alldtl.salary_max IS NULL AND alldtl.salary_min <= ? OR alldtl.salary_min IS NULL AND alldtl.salary_max >= ? OR
                    (alldtl.salary_min <= ? AND alldtl.salary_max >= ? OR alldtl.salary_min <= ? AND alldtl.salary_max >= ?)
                ) OR 
                alldtl.currency <> ? AND (
                    alldtl.salary_max IS NULL AND alldtl.salary_min <= ? OR alldtl.salary_min IS NULL AND alldtl.salary_max >= ? OR
                    (alldtl.salary_min <= ? AND alldtl.salary_max >= ? OR alldtl.salary_min <= ? AND alldtl.salary_max >= ?)
            ...
            $searchCondition['arrval'][] = $arrSearchData['currency'];
            $searchCondition['arrval'][] = $arrSearchData['salary_max'];
            $searchCondition['arrval'][] = $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $arrSearchData['salary_max'];
            $searchCondition['arrval'][] = $arrSearchData['salary_max'];
            $searchCondition['arrval'][] = $arrSearchData['currency'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_max'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_min'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_max'];
            $searchCondition['arrval'][] = $rate * $arrSearchData['salary_max'];
        } else {
            if ($arrSearchData['salary_min']) {
                $searchCondition['where'] .= '
                    alldtl.currency = ? AND (alldtl.salary_max IS NULL OR alldtl.salary_max >= ?) OR 
                    alldtl.currency <> ? AND (alldtl.salary_max IS NULL OR alldtl.salary_max >= ?)';
                $searchCondition['arrval'][] = $arrSearchData['currency'];
                $searchCondition['arrval'][] = $arrSearchData['salary_min'];
                $searchCondition['arrval'][] = $arrSearchData['currency'];
                $searchCondition['arrval'][] = $rate * $arrSearchData['salary_min'];
            }
            if ($arrSearchData['salary_max']) {
                $searchCondition['where'] .= '
                    alldtl.currency = ? AND (alldtl.salary_min IS NULL OR alldtl.salary_min <= ?) OR 
                    alldtl.currency <> ? AND (alldtl.salary_min IS NULL OR alldtl.salary_min <= ?)';
                $searchCondition['arrval'][] = $arrSearchData['currency'];
                $searchCondition['arrval'][] = $arrSearchData['salary_max'];
                $searchCondition['arrval'][] = $arrSearchData['currency'];
                $searchCondition['arrval'][] = $rate * $arrSearchData['salary_max'];
    ...
    public function doJson() {
        ...
        foreach ($this->arrProducts as $key => $val) {
            $this->arrProducts[$key]['main_large_image'] = SC_Utils_Ex::sfNoImageMainList($val['main_large_image']);
        }
