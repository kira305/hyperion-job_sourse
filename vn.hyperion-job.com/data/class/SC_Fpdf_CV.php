...
class SC_Fpdf_CV extends SC_Helper_FPDI {
    ...
    public function __construct() {
        $this->FPDF();
        // デフォルトの設定
        $this->tpl_pdf = PDF_TEMPLATE_REALDIR . 'cvvn.pdf';  // テンプレートファイル
    ...
    public function setData() {
        ...
        {
            $this->Image(IMAGE_SAVE_REALDIR . $this->customer_data['image'], 161, 20, 30 ,30);
        }
        //職歴のページ数を判定
        ...
        $this->lfText($x, $y + 2 * $lineHeight, $this->customer_data['day'] . '/' . $this->customer_data['month'] . '/' . $this->customer_data['year'], $fontSize, '');
        ...
        //卒業年月日　TODO
        $lineHeight = 5.3;
        $this->lfText($x, $y + 9 * $lineHeight, $this->customer_data['graduation_day'] . '/' . $this->customer_data['graduation_month'] . '/' . $this->customer_data['graduation_year'], $fontSize, '');
        $lineHeight = 5.3;
        ...
        if ($this->customer_data['current_salary'] != '' && $this->customer_data['current_salary'] > 1)
            $this->lfText($x, $y, $this->customer_data['current_salary'] . ' Van Yen', $fontSize, '');
        if ($this->customer_data['desired_salary'] != '' && $this->customer_data['desired_salary'] > 1)
            $this->lfText($x, $y + 1 * $lineHeight, $this->customer_data['desired_salary'] . ' Van Yen', $fontSize, '');
        $this->lfText($x, $y + 2 * $lineHeight, $this->arrIdToStrValue($this->arrRegion, $this->customer_data['desired_region']), $fontSize, '');

        $y = 153;
        $this->lfText($x, $y, 'JLPT：' . $this->arrJLPT[$this->customer_data['jp_level']] . '　　Khac：' . $this->customer_data['jp_other'], $fontSize, '');
        $this->lfText($x, $y + $lineHeight, 'TOEIC：' . $this->customer_data['toeic'] . '　　IELTS：' . $this->customer_data['ielts'] . '　　Khac：' . $this->customer_data['eng_other'], $fontSize, '');
        //他の言語
        $y = 160;
        $this->SetXY($x - 1, $y);
        ...
        for ($i = $startNo; $i < $endNo; $i++) {
            ...
            if ($this->customer_data['start_year'][$i] > 0 || $this->customer_data['end_year'][$i] > 0) {
                if ($this->customer_data['start_year'][$i] > 0)
                    $workingDate = $this->customer_data['start_month'][$i] . '/' . $this->customer_data['start_year'][$i];
                else
                    $workingDate = '--';
                ...
                if ($this->customer_data['end_year'][$i] > 0)
                    $workingDate = $this->customer_data['end_month'][$i] . '/' . $this->customer_data['end_year'][$i];
                else
                    $workingDate = '--';
                $this->SetXY($x, $y+10);
                $this->MultiCell(50, $lineHeight, $workingDate, 0, 'L', false);
