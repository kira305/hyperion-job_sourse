...
class SC_DB_MasterData {
    ...
    public $arrVnMasterDataName = array('mtb_category', 'mtb_city', 'mtb_currency', 'mtb_education', 'mtb_employment_status', 'mtb_mail_magazine_type', 'mtb_marital_status', 'mtb_object', 'mtb_position', 'mtb_product_list_max', 'mtb_pref', 'mtb_process', 'mtb_region', 'mtb_reminder', 'mtb_salary_type', 'mtb_status', 'mtb_welfare', 'mtb_sex');
    ...
    public function createCache($name, $columns = array(), $isDefine = false, $commentColumn = array()) {
        // マスターデータを取得
        if (in_array($name, $this->arrVnMasterDataName))
            $masterData = $this->getDbMasterData($name, $this->columns_vn);
        else
