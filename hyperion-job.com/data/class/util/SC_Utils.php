...
class SC_Utils {
    ...
    public function sfBriefPrice($price, $currency) {
        if (SC_Utils_Ex::isBlank($price)) {
            return '';
        }
    ...
    public function sfArrCombine($arrKeys, $arrValues) {
        if (count($arrKeys) <= 0 and count($arrValues) <= 0)
            return array();
    ...
    public static function sfGetBase64ImageData($pathImg)
    {
    ...
    public static function extendTimeOut($seconds = null) {
        ...
        if (is_null($seconds)) {
            $seconds = is_numeric(ini_get('max_execution_time')) ? intval(ini_get('max_execution_time')) : intval(get_cfg_var('max_execution_time'));
        }
