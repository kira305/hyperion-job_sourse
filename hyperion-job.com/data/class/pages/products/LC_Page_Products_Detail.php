...
class LC_Page_Products_Detail extends LC_Page_Ex {
    ...
    public function init() {
        ...
        $this->arrProcessImage = $masterData->getMasterData('mtb_process_image');
    ...
    public function action() {
        ...
        // ファイル管理クラス
        $this->objJobImg = new SC_UploadFile_Ex(IMAGE_TEMP_URLPATH_COMMON, JOB_IMAGE_SAVE_DIR_COMMON);
        // ファイル情報の初期化
        $this->objJobImg = $this->lfInitFile($this->objJobImg);
        ...
        switch ($this->mode) {
            case 'cart_to_shopping':
                ...
                if (count($this->arrErr) == 0) {
                    ...
                    $product_class_id = $this->tpl_product_class_id;
                ...
                break;
            case 'cart':
                CheckedItems::addCookie($product_id);
                break;
            case 'add_favorite':
        ...
        // 画像ファイル指定がない場合の置換処理
        $this->arrProduct['main_large_image'] = SC_Utils_Ex::sfNoImageMain($this->arrProduct['main_large_image']);

        $this->subImageFlag = $this->lfSetFile($this->objJobImg, $this->arrProduct, $this->arrFile);
        //レビュー情報の取得
        ...
        if ($objCustomer->isLoginSuccess() === true) {
            ...
            $this->isApplied = SC_Helper_DB_Ex::sfDataExists('dtb_order_detail as T1 LEFT JOIN dtb_order as T2 ON T1.order_id = T2.order_id', 'T2.del_flg = 0 AND customer_id = ? AND product_id = ?', array($objCustomer->getValue('customer_id'), $product_id));
            $this->arrProduct['is_favorite'] = $this->is_favorite;
        }

        $objQuery = & SC_Query_Ex::getSingletonInstance();
        $arrConcierge = $objQuery->select('*', 'dtb_member', 'del_flg = 0 AND authority = 2 AND member_id = ?', array($this->arrProduct['concierge']));
        if (count($arrConcierge) > 0)
            $this->arrProduct['concierge_info'] = $arrConcierge[0];
        $this->arrCheckedItems = CheckedItems::getItemList();

        $this->arrProduct['base64_image'] = '';
        if($this->arrProduct['main_large_image'] != 'noimage_main.png'){
            if(file_exists(JOB_IMAGE_SAVE_DIR_COMMON.$this->arrProduct['main_large_image'])){
                $this->arrProduct['base64_image'] = SC_Utils_Ex::sfGetBase64ImageData(JOB_IMAGE_SAVE_DIR_COMMON.'noimage_main.png');
            }
        }
        if($this->arrProduct['base64_image'] == '')   $this->arrFile['main_large_image']['filepath'] = IMAGE_SAVE_URLPATH.'noimage_main.png';
    }
    ...
    public function lfInitFile(SC_UploadFile $objUpFile) {
        $objUpFile->addFile('詳細-メイン画像', 'main_large_image', array('jpg'), IMAGE_SIZE);
    ...
    public function lfInitParam(SC_FormParam &$objFormParam) {
        ...
        $objFormParam->addParam('数量', 'quantity', INT_LEN, 'n', array('EXIST_CHECK', 'ZERO_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'), 1);
    ...
    public function lfSetFile(SC_UploadFile $objUpFile, $arrProduct, &$arrFile) {
        ...
        // ファイル表示用配列を渡す
        $arrFile = $objUpFile->getFormFileList(IMAGE_TEMP_URLPATH_COMMON, JOB_IMAGE_SAVE_DIR_COMMON, true);
    ...
    public function doAddFavorite(SC_Customer &$objCustomer) {
        // ログイン中のユーザが商品をお気に入りにいれる処理
        $this->objFormParam->setParam(array('favorite_product_id' => $this->objFormParam->getValue('product_id')));
