...
class LC_Page_Admin_Products_Product extends LC_Page_Admin_Products_Ex {
    ...
    public function action() {
        ...
        // アップロードファイル情報の初期化
        $objJobImg = new SC_UploadFile_Ex(IMAGE_TEMP_URLPATH_COMMON, JOB_IMAGE_SAVE_DIR_COMMON);
        $this->lfInitFile($objJobImg);
        $objJobImg->setHiddenFileList($_POST);
        ...
        switch ($mode) {
            ...
            case 'copy' :
                ...
                // 求人データ取得
                $arrForm = $this->lfGetFormParam_PreEdit($objJobImg, $objDownFile, $product_id);
                ...
                // ページ表示用パラメーター設定
                $this->arrForm = $this->lfSetViewParam_InputPage($objJobImg, $objDownFile, $arrForm);
                ...
                if ($mode == 'copy') {
                    ...
                    // 画像ファイルのコピー
                    $this->lfCopyProductImageFiles($objJobImg);
                }
            ...
            case 'edit':
                ...
                // エラーチェック
                $this->arrErr = $this->lfCheckError_Edit($objFormParam, $objJobImg, $objDownFile, $arrForm);
                if (count($this->arrErr) == 0) {
                    // 確認画面表示設定
                    $this->tpl_mainpage = 'products/confirm.tpl';
                    $this->arrCatList = $this->lfGetCategoryList_Edit();
                    $this->arrForm = $this->lfSetViewParam_ConfirmPage($objJobImg, $objDownFile, $arrForm);
                } else {
                    // 入力画面表示設定
                    $this->arrForm = $this->lfSetViewParam_InputPage($objJobImg, $objDownFile, $arrForm);
                    // ページonload時のJavaScript設定
            ...
            case 'complete':
                ...
                if (count($this->arrErr) == 0) {
                    // DBへデータ登録
                    $product_id = $this->lfRegistProduct($objJobImg, $objDownFile, $arrForm);
                    ...
                    $this->lfSaveUploadFiles($objJobImg, $objDownFile, $product_id);
            ...
            case 'delete_image':
                ...
                switch ($mode) {
                    case 'upload_image':
                        ...
                        $this->arrErr[$arrForm['image_key']] = $objJobImg->makeTempFile($arrForm['image_key'], IMAGE_RENAME);
                        if ($this->arrErr[$arrForm['image_key']] == '') {
                            // 縮小画像作成
                            $this->lfSetScaleImage($objJobImg, $arrForm['image_key']);
                        }
                        break;
                    case 'delete_image':
                        // ファイル削除
                        $this->lfDeleteTempFile($objJobImg, $arrForm['image_key']);
                        break;
                    default:
                        break;
                }
...
Thông tin đơn giản về bộ phận kinh doanh：' . $client['introduction_vn']);
     ...
     * - 入力画面
     *
     * @param  SC_UploadFile_Ex $objJobImg   SC_UploadFileインスタンス
     * @param  SC_UploadFile_Ex $objDownFile SC_UploadFileインスタンス
     * @param  array  $arrForm     フォーム入力パラメーター配列
     * @return array  表示用フォームパラメーター配列
     */
    public function lfSetViewParam_InputPage(&$objJobImg, &$objDownFile, &$arrForm) {
        // カテゴリマスターデータ取得
        $objDb = new SC_Helper_DB_Ex();
        ...
        $arrHidden = $objJobImg->getHiddenFileList();
        $arrForm['arrHidden'] = array_merge((array) $arrHidden, (array) $objDownFile->getHiddenFileList());

        // 画像ファイル表示用データ取得
        $arrForm['arrFile'] = $objJobImg->getFormFileList(IMAGE_TEMP_URLPATH_COMMON, JOB_IMAGE_SAVE_DIR_COMMON);
    ...
    public function lfSetViewParam_ConfirmPage(&$objUpFile, &$objDownFile, &$arrForm) {
        // 画像ファイル用データ取得
        $arrForm['arrFile'] = $objUpFile->getFormFileList(IMAGE_TEMP_URLPATH_COMMON, JOB_IMAGE_SAVE_DIR_COMMON);
        // ダウンロード求人実ファイル名取得
        $arrForm['down_realfilename'] = $objDownFile->getFormDownFile();
