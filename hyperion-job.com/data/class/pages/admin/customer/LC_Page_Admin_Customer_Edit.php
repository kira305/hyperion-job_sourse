...
class LC_Page_Admin_Customer_Edit extends LC_Page_Admin_Ex
{
    ...
    public function action()
    {
        ...
        $objAvtFile = new SC_UploadFile_Ex(IMAGE_TEMP_URLPATH_COMMON, IMAGE_SAVE_DIR_COMMON);
        ...
        switch ($this->getMode()) {
            case 'edit_search':
                ...
                // 購入履歴情報の取得
                list($this->tpl_linemax, $this->arrAppliedHistory, $this->objNavi) = $this->lfAppliedHistory($objFormSearchParam->getValue('edit_customer_id'));
            ...
            case 'return':
                ...
                // 購入履歴情報の取得
                list($this->tpl_linemax, $this->arrAppliedHistory, $this->objNavi) = $this->lfAppliedHistory($objFormParam->getValue('customer_id'), $objFormParam->getValue('search_pageno'));
                $this->arrPagenavi = $this->objNavi->arrPagenavi;
                $this->arrPagenavi['mode'] = 'return';
    ...
    public function setUploadFile(&$objUpFile, &$arrForm)
    {
        ...
        $arrForm['arrFile'] = $objUpFile->getFormFileList(IMAGE_TEMP_URLPATH_COMMON, IMAGE_SAVE_DIR_COMMON);
    }
    ...
    public function lfRegistData(&$objFormParam, $objUpFile)
    {
        ...
        $arrData['graduation_date'] = $arrData['graduation_year'].'-'.$arrData['graduation_month'].'-'.$arrData['graduation_day'];
     ...
     */
    public function lfAppliedHistory($customer_id, $pageno = 0)
    {
        if (SC_Utils_Ex::isBlank($customer_id)) {
            return array('0', array(), NULL);
        }
        $objQuery = &SC_Query_Ex::getSingletonInstance();
        $objQueryOther =& SC_Query_Ex::getSingletonInstance();
        $arrayEmpStatus = array(1=>'正社員',2=>'アルバイト',3=>'派遣');
        ...
        $sql_product_id = $objQuery->getSql('product_id', 'dtb_order_detail', 'order_id = dtb_order.order_id');
        $sql_product_id = $objQuery->dbFactory->addLimitOffset($sql_product_id, 1);
        $cols = <<< __EOS__
            dtb_order.order_id,
            dtb_order.customer_id,
            dtb_order.create_date,
            ($sql_product_id) AS product_id
__EOS__;
        // 購入履歴情報の取得
//        $arrPurchaseHistory = $objQuery->select('*', $table, $where, $arrVal);

        $arrApply = $objQuery->select($cols, $table, $where, $arrVal);
        //get array all category
        $allCategory = $objQueryOther->select('id, name', 'mtb_category');
        $arrCate = array();
        foreach ($allCategory as $key => $cateItem) {
            $arrCate[$cateItem['id']] = $cateItem['name'];
        }

        foreach ($arrApply as $key => $val) {
            $productData = $objQueryOther->select('name, employment_status, category_id', 'dtb_products', 'product_id = ?', array($val['product_id']));
            $customerName = $objQueryOther->select('name01, name02', 'dtb_customer', 'customer_id = ?', array($val['customer_id']));
            //get category name to show
            $arrCateId = explode(' ',$productData[0]['category_id']);
            $arrApply[$key]['category_name'] = $this->getCateName($arrCateId, $arrCate);
            //get employment status to show
            $arrApply[$key]['employment_status'] = $arrayEmpStatus[$productData[0]['employment_status']];
            //get customer name to show
            $arrApply[$key]['customer_name'] = $customerName[0]['name01'].' '.$customerName[0]['name02'];
            //get job name to show
            $arrApply[$key]['job_name'] = $productData[0]['name'];

            $arrApply[$key]['create_date'] = str_replace('-', '/', substr($val['create_date'], 0, 19));
        }
        return array($linemax, $arrApply, $objNavi);
    }

    private function getCateName($arrId, $arrCategory)
    {
        $strCateName = '';
        foreach ($arrId as $id) {
            $strCateName .= '・'.$arrCategory[$id].'</br>';
        }
        return $strCateName;
    }
