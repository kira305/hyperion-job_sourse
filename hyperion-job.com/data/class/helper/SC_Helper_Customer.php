...
class SC_Helper_Customer {
        ...
        if (isset($arrParams['working_company_name'])) {
            ...
            foreach ($arrParams['working_company_name'] as $index => $working_company_name) {
                if ($working_company_name != '') {
                    ...
                    $arrCareer['working_member_num'] = $arrParams['working_member_num'][$index];
                    $arrCareer['working_status'] = $arrParams['working_status'][$index];
                    $arrCareer['working_position'] = $arrParams['working_position'][$index];
                    $arrCareer['working_type'] = $arrParams['working_type'][$index];
                    $arrCareer['working_experience'] = $arrParams['working_experience'][$index];
    ...
    public function sfGetCustomerData($customer_id, $mask_flg = true) {
        ...
        $career = $objQuery->select('start_date, end_date, working_company_name, company_addr, working_member_num, working_status, working_position, working_type, working_experience, job_description, working_year', 'dtb_customer_career', 'customer_id=?', array($customer_id));
        ...
        // 卒業日を年月日に分ける
        if (isset($arrForm['graduation_date'])) {
            $graduation_date = explode(' ', $arrForm['graduation_date']);
            list($arrForm['graduation_year'], $arrForm['graduation_month'], $arrForm['graduation_day']) = explode('-', $graduation_date[0]);
        }
    ...
    public function sfGetResumeInfo($customer_id) {
        $objQuery = & SC_Query_Ex::getSingletonInstance();

        $col = "resume, resume_name";
        $from = "dtb_customer";
        $where = 'customer_id = ?';
        $objQuery = new SC_Query_Ex();
        $arrRet = $objQuery->select($col, $from, $where, array($customer_id));

        if (!empty($arrRet[0]["resume"])) {
            return $arrRet[0];
        }
        return false;
    }
...
//        $objFormParam->addParam('電話番号', $prefix . 'tel', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false, $prefix = '') {
        ...
        $objFormParam->addParam('配偶者', $prefix . 'marital_status', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('卒業日・年', $prefix . 'graduation_year', 4, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('卒業日・月', $prefix . 'graduation_month', 2, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('卒業日・日', $prefix . 'graduation_day', 2, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('直近年収', $prefix . 'current_salary', PRICE_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK', 'ZERO_START'));
        $objFormParam->addParam('希望年収', $prefix . 'desired_salary', PRICE_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK', 'ZERO_START'));
        ...
        $objFormParam->addParam('従業員数', $prefix . 'working_member_num', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('雇用形態', $prefix . 'working_status', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('部署・役職', $prefix . 'working_position', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('業種', $prefix . 'working_type', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('経験職種', $prefix . 'working_experience', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('仕事概要', $prefix . 'job_description', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function sfCustomerResumeParam(&$objFormParam) {
        $objFormParam->addParam('職務履歴書ファイル名', 'resume', MTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('職務履歴書ファイル名', 'resume_name');
    ...
    public function sfCustomerMypageErrorCheck(&$objFormParam, $isAdmin = false) {
        ...
        if ($arrValues['work_experience'] == 1) {
            $objError1 = new SC_CheckError_Ex(array(
                ...
                'working_member_num' => $arrValues['working_member_num'][0],
                'working_status' => $arrValues['working_status'][0],
                'working_position' => $arrValues['working_position'][0],
                'working_type' => $arrValues['working_type'][0],
                'working_experience' => $arrValues['working_experience'][0],
            ...
            $objError1->doFunc(array('従業員数', 'working_member_num'), array('EXIST_CHECK'));
            $objError1->doFunc(array('雇用形態', 'working_status'), array('EXIST_CHECK'));
            $objError1->doFunc(array('部署・役職', 'working_position'), array('EXIST_CHECK'));
            $objError1->doFunc(array('業種', 'working_type'), array('EXIST_CHECK'));
            $objError1->doFunc(array('経験職種', 'working_experience'), array('EXIST_CHECK'));
            $objError1->doFunc(array('仕事概要', 'job_description'), array('EXIST_CHECK'));
            if (count($objError1->arrErr) > 0) {
                ...
                $arrErr['working_member_num'][0] = $objError1->arrErr['working_member_num'];
                $arrErr['working_status'][0] = $objError1->arrErr['working_status'];
                $arrErr['working_position'][0] = $objError1->arrErr['working_position'];
                $arrErr['working_type'][0] = $objError1->arrErr['working_type'];
                $arrErr['working_experience'][0] = $objError1->arrErr['working_experience'];
...
//        $objFormParam->addParam('購入回数(終了)', 'search_buy_times_to', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('最終応募日(開始年)', 'search_buy_start_year', 4, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終応募日(開始月)', 'search_buy_start_month', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終応募日(開始日)', 'search_buy_start_day', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終応募日(終了年)', 'search_buy_end_year', 4, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終応募日(終了月)', 'search_buy_end_month', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終応募日(終了日)', 'search_buy_end_day', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
//        $objFormParam->addParam('購入商品コード', 'search_buy_product_code', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('求人ID', 'search_product_id', STEXT_LEN, 'KVa', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('求人名', 'search_product_name', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
     ...
     */
     public function sfGetSearchData($arrParam, $limitMode = '') {
        ...
        foreach ($arrData as $key => $obj) {
            // check cv exist
            if (!empty($obj['cv'])) {
                if (file_exists(DOWN_SAVE_DIR_COMMON . $obj["cv"]))
            ...
            if (!empty($obj['resume'])) {
                ...
                if (file_exists(DOWN_SAVE_DIR_COMMON . $obj["resume"]))
                    $arrData[$key]['existsResume'] = true;
                else
                    $arrData[$key]['existsResume'] = false;
            } else {
                $arrData[$key]['existsResume'] = false;
            }
            //current country
            if (!empty($obj['current_address'])) {
                $arrData[$key]['current_address'] = $curCountry[$obj['current_address']];
            }
