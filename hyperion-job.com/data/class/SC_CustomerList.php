...
//        }
        ...
        // 求人名
        if (!isset($this->arrSql['search_product_name'])) $this->arrSql['search_product_name'] = '';
        if (strlen($this->arrSql['search_product_name']) > 0) {
            ...
            $search_buyproduct_name = $this->addSearchStr($this->arrSql['search_product_name']);
            $this->arrVal[] = $search_buyproduct_name;
        }

        // 求人ID
        if (!isset($this->arrSql['search_product_id'])) $this->arrSql['search_product_id'] = '';
        if (strlen($this->arrSql['search_product_id']) > 0) {
            $this->setWhere('customer_id IN (SELECT customer_id FROM dtb_order WHERE order_id IN (SELECT order_id FROM dtb_order_detail WHERE product_id LIKE ?) AND del_flg = 0)');
            $search_buyproduct_name = $this->addSearchStr($this->arrSql['search_product_id']);
            $this->arrVal[] = $search_buyproduct_name;
        }
    ...
    {
        $this->select = 'SELECT customer_id,name01,name02,kana01,kana02,sex,email,email_mobile,tel01,tel02,tel03,pref,status,update_date,mailmaga_flg,pref_by_text,tel FROM dtb_customer ';
    ...
    {
        $this->select = 'SELECT customer_id,name01,name02,kana01,kana02,sex,email,email_mobile,tel01,tel02,tel03,pref,status,update_date,mailmaga_flg,pref_by_text,tel,cv,cv_name,resume,resume_name,current_address FROM dtb_customer ';
